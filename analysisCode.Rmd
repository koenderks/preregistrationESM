...
.plotFixedCoefficients <- function(fit, title, xmin, xmax, warmup){
  mtfit <- ggs(fit)
  sumFixed <- summary(fit)$fixed
  p <- ggplot(filter(mtfit, Parameter %in% c("b_Intercept", "b_PP","b_AGE"), Iteration > warmup),
              ...
              scale_y_continuous(name = "Density") +
              geom_segment(x = sumFixed[1,3], xend = sumFixed[1,3], y = 0, yend = Inf, col = "dodgerblue", linetype = 2) +
              geom_segment(x = sumFixed[1,4], xend = sumFixed[1,4], y = 0, yend = Inf, col = "dodgerblue", linetype = 2) +
              geom_segment(x = sumFixed[2,3], xend = sumFixed[2,3], y = 0, yend = Inf, col = "red", linetype = 2) +
              geom_segment(x = sumFixed[2,4], xend = sumFixed[2,4], y = 0, yend = Inf, col = "red", linetype = 2) +
              geom_segment(x = sumFixed[3,3], xend = sumFixed[3,3], y = 0, yend = Inf, col = "darkgreen", linetype = 2) +
              geom_segment(x = sumFixed[3,4], xend = sumFixed[3,4], y = 0, yend = Inf, col = "darkgreen", linetype = 2) +
              geom_segment(x = xmin, xend = xmax, y = -Inf, yend = -Inf) +
              geom_segment(x = -Inf, xend = -Inf, y = 0, yend = 20) +
              scale_fill_manual(name   =  'Parameters', 
                               values = c("darkgreen","dodgerblue", "red"), 
...
Below we will outline six models in order to assert the evidence in the data for these hypotheses.
...
$$SHAREN_{ij} = \gamma_{00} + \gamma_{10} \cdot PP_{ij} + \gamma_{20} \cdot AGE_{ij} + \gamma_{30} \cdot GEN_{{M}_{ij}} + u_{4j} \cdot INEG_{ij} + \epsilon_{ij},$$

in which observation $i$ from participant $j$ is predicted.
...
p <- ggplot() +
      ...
      labs(title = expression(paste("ID Level Error of Regression Coefficient of INEG on SHARIN (", u["4j"],")")),
           subtitle = expression(paste("Posterior distribution of ID level error of regression coefficient (", u["4j"],") per ID with the means in red"))) +
      theme_minimal() +
      theme(panel.grid = element_blank(), axis.ticks = element_line())
print(p)
```
