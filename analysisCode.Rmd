...
p1 <- ggplot(data = dataset, aes(x = PP, y = SHAREN)) +
        ...
        labs(title = "Sharing vs. Psychopathology",
              subtitle = "Fixed effect") +
...
p2 <- ggplot(data = dataset, aes(x = INEG, y = SHAREN, col = ID, group = ID)) +
        ...
        geom_smooth(method = lm, se = FALSE, size = .5, alpha = .8) +
        labs(title = "Sharing vs. Intensity of negative events",
...
results$pval[c(1, 6)] <- c(test1_f$`Pr(>Chisq)`[6],
                           test1_f$`Pr(>Chisq)`[9])
results$er[c(1, 6)] <- c(hypothesis1_bi$hypothesis[6], 
                          hypothesis6_bi$hypothesis[6])
print(results)
```
